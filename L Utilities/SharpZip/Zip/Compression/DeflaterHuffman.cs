    ...
    public class DeflaterHuffman
        ...
        private class Tree
            ...
            public void CheckEmpty()
                {
                bool empty = true;
                foreach (short t in this.freqs)
                    {
                    if (t != 0)
                        {
                        //Console.WriteLine($"freqs[{i}] == {this.freqs[i]}");
                        empty = false;
                        }
                    }
            ...
            public void BuildCodes()
                ...
                for (int bits = 0; bits < this.maxLength; bits++)
                    ...
                    //					if (DeflaterConstants.DEBUGGING) {
                    //						Console.WriteLine($"bits: {(bits + 1)} count: {this.bl_counts[bits]} nextCode: {code}");
                    //					}
                    }
                ...
                for (int i = 0; i < this.numCodes; i++)
                    ...
                    if (bits > 0)
                        ...
                        //						if (DeflaterConstants.DEBUGGING) {
                        //								Console.WriteLine($"codes[{i}] = rev({nextCode[bits - 1]}{"),}{bits}");
                        //						}
            ...
            public int GetEncodedLength()
                {
                return this.freqs.Select((t, i) => t * this.length[i]).Sum();
                }
            ...
            private void BuildLength(IReadOnlyList<int> childs)
                {
                this.length = new byte[this.freqs.Length];
                int numNodes = childs.Count / 2;
                ...
                //						Console.WriteLine(
                //						    $"Node {childs[2*i]} freq: {this.freqs[childs[2*i]]} len: {this.length[childs[2*i]]}");
                //					}
                //				}
        ...
        public void FlushBlock(byte[] stored, int storedOffset, int storedLength, bool lastBlock)
            ...
            if (storedOffset >= 0 && storedLength + 4 < opt_len >> 3)
                ...
                //				if (DeflaterConstants.DEBUGGING) {
                //					Console.WriteLine($"Storing, since {storedLength} < {opt_len} <= {static_len}");
                //				}
