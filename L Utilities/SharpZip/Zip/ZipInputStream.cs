    ...
    public class ZipInputStream : InflaterInputStream
        ...
        public ZipEntry GetNextEntry()
            ...
            if (this.entry.IsCompressionMethodSupported())
                {
                this.internalReader = this.InitialRead;
                }
            else
                {
                this.internalReader = this.ReadingNotSupported;
                }
        ...
        private int InitialRead(byte[] destination, int offset, int count)
            ...
            if (this.entry.IsCrypted)
                ...
                if (this.csize >= ZipConstants.CryptoHeaderSize)
                    ...
                    {
                    throw new ZipException($"Entry compressed size {this.csize} too small for encryption");
                    }
#endif
                }
            else
            ...
            if ((this.csize > 0) || ((this.flags & (int)GeneralBitFlags.Descriptor) != 0))
                ...
                this.internalReader = this.BodyRead;
                    ...
                    if (count <= 0)
                        ...
                        if ((this.flags & 8) == 0 && (this.inf.TotalIn != this.csize || this.inf.TotalOut != this.size))
                            ...
                            throw new ZipException(
                                $"Size mismatch: {this.csize};{this.size} <-> {this.inf.TotalIn};{this.inf.TotalOut}");
                            }
                        this.inf.Reset();
                        finished = true;
                        }
