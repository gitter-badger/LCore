...
namespace ICSharpCode.SharpZipLib.Tar
    ...
    public class TarHeader : ICloneable
        ...
        public override bool Equals([CanBeNull]object obj)
            ...
            if (localHeader != null)
                {
                return this.name == localHeader.name
        ...
        private static int ComputeCheckSum(IReadOnlyCollection<byte> buffer)
            {
            int sum = 0;
        ...
        private static int MakeCheckSum(IReadOnlyList<byte> buffer)
            {
            int sum = 0;
            ...
            for (int i = CHKSUMOFS + CHKSUMLEN; i < buffer.Count; ++i)
                {
                sum += buffer[i];
                }
            return sum;
