...
namespace LCore.Extensions
    ...
    public static class ReflectionExt
        ...
        public static string GetFriendlyTypeName([CanBeNull] this Type In)
            {
            if (In?.Namespace == "System.Data.Entity.DynamicProxies" || In?.Name.Contains("Proxy") == true)
                In = In.BaseType;

            if (In == null)
                return "";

            if (In.IsGenericType && In.Name.Has(Obj: '`'))
                {
                string Out = "";
        ...
        public static string FindClassFile([CanBeNull] this Type Type)
            ...
            Func<Type, string> Cache = new Func<Type, string>(CacheType =>
                ...
                if (Directory.Exists(Path))
                    {
                    // Ensure we get the root class file name for nested types
                    while (CacheType.DeclaringType != null)
                        CacheType = CacheType.DeclaringType;
        ...
        /// </summary>
        public static bool IsProtected([CanBeNull] this Type Type)
            {
            return Type != null && !Type.IsPublic && Type.IsNestedFamily;
            }
        ...
        /// <summary>
        /// Determines if a <paramref name="Type"/> is marked as internal.
        /// </summary>
        public static bool IsInternal([CanBeNull] this Type Type)
            {
            return Type != null && !Type.IsPublic && Type.IsNestedAssembly;
            }
        }
